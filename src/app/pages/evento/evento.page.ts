...
export class EventoPage implements OnInit {
  ...
  inscripcion: any = true;
  asistencia: any = true;
  cargando: any = false;
  cargandoEvento: boolean = false;
  vistaAsistencia: boolean = true;
  ...
  async llamarEvento(accion: string, evento: string, documento: string) {
    ...
    await this.srSoftExpert
      .getEvento(accion, evento, documento)
      .pipe(
        tap((response) => {
  ...
  verEventoQr(fn: string, documento: string, rol: string, id: string) {
    this.uwService.consultaEvento(fn, documento, rol, id).subscribe((data) => {
      this.qrCode = data.qr;
    });
  }
  ...
  cancelarEvento() {
    this.cargando = true;
    this.uwService.calcelarEvento(this.idEvento).subscribe((data) => {
      this.cargando = false;
      if (data.resp) {
        this.msgService.presentToastMsg(
          `Evento ${this.arrayEvento.actividad} cancelado`,
          'danger'
        );
        this.location.back();
        // this.navCtrl.navigateForward('/tabs/eventos');
  ...
  consultarTipoDocumento() {
    this.uwService.consultarDocumento().subscribe((data) => {
